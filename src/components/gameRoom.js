...
export default function GameRoom(props) {
  ...
  const [numberOfQuestions, setNumberOfQuestion] = useState(0) // number of question should be passed inside "props" when generate request
  ...
  const tempHandleRightAnswer = () => {
    (
      ...
      console.log("Answer correctly, and now at question-nth: ", nthQuestion),
  ...
  return (
    ...
    !insideAQuestion
      ? !(winner && gameOver)
        ? // if not Won or Lost the game => Just created 
        <View>
          <Text onPress={() => tempGetQuestion()}>Click Here When Ready!</Text>
        </View>
        : winner // If the user won the game
          ?
          <View>
        ...
        <View>
          {
            questionData.type == 'picture'
              ...
              : questionData.type == 'hangman'
                ? <Hangman data={questionData} rightAnswer={() => { tempHandleRightAnswer() }} />
                : questionData.type == 'riddle'
                  ? <Riddle data={questionData} rightAnswer={() => { tempHandleRightAnswer() }}/>
                  : console.log('works')
          }
        </View>
